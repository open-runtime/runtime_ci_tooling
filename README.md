...
- **Multi-Platform CI**: Multi-platform CI workflow generation supporting configurable platform matrices via `config.json`'s `ci.platforms` array, configurable artifact retention, `ci.runner_overrides` config, and configurable Git URL rewrite orgs via `ci.git_orgs`.
...
- **Template Updating**: Keep local configurations and CI workflows in sync with upstream changes. Includes support for previewing changes with `--diff`.
...
As of version **v0.16.0**, tools are available as executables in `bin/` (and globally), and CLI options are strictly typed.
...
- `update`: Update templates, configs, and workflows from runtime_ci_tooling. Use `--diff` to preview changes.
...
- `autodoc`: Generate/update module documentation with package-aware output pathing and generated `docs/README.md` index. Use `--init` to automatically scaffold `autodoc.json` based on the `lib/src/` structure.
