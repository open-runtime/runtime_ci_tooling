...
void main() {
  group('TestCommand.runWithRoot', () {
    ...
    void writeSubPackageConfig(List<Map<String, String>> subPackages) {
      final configDir = Directory(p.join(tempDir.path, '.runtime_ci'))
    ...
    test(
      'skips root tests and succeeds when no test/ directory exists',
      () async {
    ...
    test(
      'runs root tests, writes results.json, and StepSummary pathway produces valid output',
      () async {
        ...
        File(
          p.join(tempDir.path, 'test', 'passing_test.dart'),
        ).writeAsStringSync('''
import 'package:test/test.dart';
...
''');
        ...
        expect(
          Directory(logDir).existsSync(),
          isTrue,
          reason: 'log dir should be created',
        );
        ...
        final hasResults =
            File(jsonPath).existsSync() || File(expandedPath).existsSync();
        expect(
          hasResults,
          isTrue,
          reason: 'at least one reporter output should exist',
        );
...
''',
    ...
    test(
      'exits when configured sub-package directory has no pubspec.yaml',
      () async {
        writeRootPubspec();
        ...
        Directory(
          p.join(tempDir.path, 'packages', 'pkg_a'),
        ).createSync(recursive: true);
    ...
    test('exits when sub-package pub get times out', () async {
      ...
      final pkgDir = Directory(p.join(tempDir.path, 'packages', 'pkg_timeout'))
        ..createSync(recursive: true);
