...
class TriageStatusCommand extends Command<void> {
  ...
  Future<void> run() async {
    ...
    // Show config
    Logger.info('  Config: ${config.repoOwner}/${config.repoName}');
    Logger.info(
    ...
    if (lockFile.existsSync()) {
      try {
        ...
        Logger.info('  Lock: ACTIVE (PID: ${lockData['pid']}, started: ${lockData['started']})');
      } catch (_) {
    ...
    if (runsDir.existsSync()) {
      ...
      for (final run in runs.take(5)) {
        ...
        Logger.info('    $name${hasCheckpoint ? " [checkpoint]" : ""}${hasPlan ? " [plan]" : ""}');
      }
    }
    ...
    for (final entry in mcpStatus.entries) {
      Logger.info('    ${entry.key}: ${entry.value ? "configured" : "NOT configured"}');
    }
  }
}
