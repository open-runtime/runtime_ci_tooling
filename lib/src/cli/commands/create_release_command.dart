...
class CreateReleaseCommand extends Command<void> {
  ...
  Future<void> run() async {
    ...
    final effectiveRepo =
        repo ?? Platform.environment['GITHUB_REPOSITORY'] ?? '';
    ...
    pubspecFile.writeAsStringSync(pubspecContent.replaceFirst(
    ...
    for (final path in releaseNotesSearchPaths) {
      ...
      if (f.existsSync() && f.lengthSync() > 100) {
        foundReleaseNotes = f;
        Logger.info(
            'Found release notes at: $path (${f.lengthSync()} bytes)');
        break;
      }
    }
    ...
    if (foundReleaseNotes != null &&
      ...
      foundReleaseNotes
          .copySync('${releaseDir.path}/release_notes.md');
      Logger.info(
          'Copied release notes to ${releaseDir.path}/release_notes.md');
    } else if (foundReleaseNotes == null) {
      File('${releaseDir.path}/release_notes.md').writeAsStringSync(
          ReleaseUtils.buildFallbackReleaseNotes(
    ...
    if (existingHighlights.existsSync()) {
      Logger.info(
          'Highlights: ${existingHighlights.lengthSync()} bytes');
    }
    ...
    if (changelog.existsSync()) {
      ...
      final entryMatch = RegExp(
              '## \\[$newVersion\\].*?(?=## \\[|\\Z)', dotAll: true)
          .firstMatch(content);
      File('${releaseDir.path}/changelog_entry.md').writeAsStringSync(
          entryMatch?.group(0)?.trim() ?? '## [$newVersion]\n');
    }

    // Contributors: use the single verified source of truth
    final contribs =
        ReleaseUtils.gatherVerifiedContributors(repoRoot, prevTag);
    File('${releaseDir.path}/contributors.json').writeAsStringSync(
        const JsonEncoder.withIndent('  ').convert(contribs));

    Logger.success(
        'Release notes assembled in $kReleaseNotesDir/v$newVersion/');

    if (global.dryRun) {
      Logger.info(
          '[DRY-RUN] Would commit, tag, and create GitHub Release');
      return;
    }
    ...
    CiProcessRunner.exec(
        ...
        cwd: repoRoot, verbose: global.verbose);
    CiProcessRunner.exec('git',
    ...
    for (final path in filesToAdd) {
      ...
      if (File(fullPath).existsSync() ||
        ...
        CiProcessRunner.exec('git', ['add', path],
            cwd: repoRoot, verbose: global.verbose);
      }
    }
    ...
    if (diffResult.exitCode != 0) {
      ...
      CiProcessRunner.exec('git', ['commit', '-F', commitMsgFile.path],
          cwd: repoRoot, fatal: true, verbose: global.verbose);
      ...
      CiProcessRunner.exec('git', ['push', 'origin', 'main'],
          cwd: repoRoot, fatal: true, verbose: global.verbose);
    ...
    final tagCheck = Process.runSync('git', ['rev-parse', tag],
        workingDirectory: repoRoot);
    ...
    CiProcessRunner.exec(
        'git', ['tag', '-a', tag, '-m', 'Release v$newVersion'],
        cwd: repoRoot, fatal: true, verbose: global.verbose);
    CiProcessRunner.exec('git', ['push', 'origin', tag],
        cwd: repoRoot, fatal: true, verbose: global.verbose);
    ...
    if (bodyFile.existsSync() && bodyFile.lengthSync() > 50) {
      ...
      releaseBody = ReleaseUtils.buildFallbackReleaseNotes(
          repoRoot, newVersion, prevTag);
    }
    ...
    CiProcessRunner.exec('gh', ghArgs,
        cwd: repoRoot, verbose: global.verbose);
    Logger.success('Created GitHub Release: $tag');

    // Build rich summary
    final rnPreview =
        FileUtils.readFileOr('${releaseDir.path}/release_notes.md');
    final clEntryContent =
        FileUtils.readFileOr('${releaseDir.path}/changelog_entry.md');
    final contribContent =
        FileUtils.readFileOr('${releaseDir.path}/contributors.json');
