...
/// Autodoc: MIGRATION.md generator for a Dart source module.
///
/// Usage:
...
void main(List<String> args) {
  ...
  final addedDefinitions = prevHash.isNotEmpty
      ? _runSync(
          'git diff $prevHash..HEAD -- $sourceDir | grep "^+" | grep "class\\|mixin\\|extension\\|enum" | head -20',
        )
      : '';
  final removedDefinitions = prevHash.isNotEmpty
      ? _runSync(
          'git diff $prevHash..HEAD -- $sourceDir | grep "^-" | grep "class\\|mixin\\|extension\\|enum" | head -20',
        )
      : '';
...
${addedDefinitions.isNotEmpty ? '## Added Definitions\n```\n$addedDefinitions\n```' : ''}
${removedDefinitions.isNotEmpty ? '## Removed Definitions\n```\n$removedDefinitions\n```' : ''}
...
### 1. Breaking Changes
For each breaking change (removed/renamed classes, methods, fields, mixins, extensions):
- What was removed/renamed
- The replacement (if any)
- Before/after code example

### 2. New Features
For each new class/method/extension added:
- What it does
- Basic usage example
